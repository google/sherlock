...
class ConnectedDevice:
  ...
  Attributes:
      adb_device (adbutils.AdbDevice): The `adbutils.AdbDevice` object
        representing the device
      sherlock_config (sherlock_config.SherlockConfig): Sherlock configuration
        settings
  """

  def __init__(
      self,
      adb_device: adbutils.AdbDevice,
      sherlock_config: sherlock_config.SherlockConfig,
  ):
  ...
  def _build_perfetto_shell_cmd(self, remote_trace_filepath: str):
    ...
    Args:
        remote_trace_filepath: The remote file path where the trace data will be
          saved.
    ...
    cmd = [
        'shell',
        self.sherlock_config.perfetto_cmd,
        '--background',
        '-o',
  ...
  def _generate_perfetto_trace_filename(self) -> str:
    ...
    trace_filename = (
        f'{timestamp}-{random_suffix}.{sherlock_config.TRACES_EXTENSION}'
    )
  ...
  def _get_running_trace_output_remote_filepath(self) -> str | None:
    ...
    try:
      ...
      logging.debug(
          '[%s]: perfetto is running with: %s', self.serial, perfetto_cmdline
      )
  ...
  def list_trace_files(self) -> list[str]:
    ...
    Returns:
        ...
        Returns an empty list if no trace files are found or if there is an
        error
    ...
    try:
      trace_files = [
          f
          for f in self.shell(
              f'ls {self.sherlock_config.trace_remote_output_dir}'
          ).split()
          if f.endswith(sherlock_config.TRACES_EXTENSION)
      ]
      return [
          f'{self.sherlock_config.trace_remote_output_dir}/{f}'
  ...
  def shell(self, cmd: str) -> str:
    ...
    Raises:
        ConnectedDeviceAdbCmdError: If there is an error communicating with the
        device or executing the command.
    """
    try:
      return self.adb_device.shell(cmd)
    except adbutils.errors.AdbError:
      logging.warning(
          '[%s]: unable to execute ADB shell <%s>', self.serial, cmd
      )
  ...
  def start_perfetto(self) -> bool:
    ...
    if self.perfetto_pid():
      logging.debug(
          '[%s] perfetto is already running with PID=%d',
          self.serial,
          self.perfetto_pid(),
      )
      return False
    ...
    with open(self.sherlock_config.trace_config_file_path, 'rb') as f:
      ...
      if match is None:
        logging.warning(
            '[%s]: failed to read the pid from `perfetto --background`',
            self.serial,
        )
        return False
      ...
      if exit_code != 0:
        ...
        return False
      logging.info(
          '[%s]: perfetto has started with PID = %s', self.serial, perfetto_pid
      )
      return True
  ...
  def stop_perfetto(self) -> None:
    ...
    if not perfetto_pid:
      ...
      logging.debug(
          '[%s]: terminate perfetto process with PID=%d',
          self.serial,
          perfetto_pid,
      )
      ...
      while self.perfetto_pid():
        time.sleep(0.5)
      logging.info(
          '[%s]: perfetto (PID=%s) has been terminated',
          self.serial,
          perfetto_pid,
      )
  ...
  def collect_traces(
    ...
    Args:
        filename_filter: A function that takes a filename as input and returns
          True if the file should be collected, False otherwise. Defaults to a
          function that accepts all files.
    ...
    local_output_dirpath = os.path.join(
        self.sherlock_config.local_output_dir, self.serial
    )
    try:
      ...
      for trace_filepath in self.list_trace_files():
        if filename_filter(trace_filepath):
          logging.debug(
              '[%s]: collecting %s trace file', self.serial, trace_filepath
          )
          ...
          logging.info(
              '[%s]: trace %s saved in %s',
              self.serial,
              trace_filepath,
              local_output_dirpath,
          )
      ...
      logging.warning(
          '[%s]: permission denied: Unable to create directory %s',
          self.serial,
          local_output_dirpath,
      )
    except OSError as e:
