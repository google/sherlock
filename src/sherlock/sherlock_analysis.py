...
The user can select the analysis modules to run using the `TraceAnalysisMode`
enum.
...
class TraceAnalysis:
  ...
  Attributes:
      sherlock_config (sherlock_config.SherlockConfig):  Sherlock configuration
        settings
      analysis_module_classes (list[trace_analysis.TraceAnalysisMode]): List of
  ...
  def __init__(
    ...
    self.analysis_module_classes: list[trace_analysis.TraceAnalysisModule] = (
  ...
  def _local_trace_filepath(self) -> dict[str, list[str]]:
    ...
    Returns:
        dict[str, list[str]]: A dictionary where the keys are the serial numbers
        and the values are the
        list of trace files associated with that serial number.
    """
    trace_filepath_by_serial: dict[str, list[str]] = collections.defaultdict(
        list
    )
    try:
      ...
      serial_dirs = [
          ...
          if os.path.isdir(
              os.path.join(self.sherlock_config.trace_local_output_dir, entry)
          )
      ]
    ...
    for serial_dir in trace_filepath_by_serial:
      try:
        serial_dir_path = os.path.join(
            self.sherlock_config.trace_local_output_dir, serial_dir
        )
        content = os.listdir(serial_dir_path)
        for entry in content:
          if os.path.isfile(
              os.path.join(serial_dir_path, entry)
          ) and entry.endswith(f'.{sherlock_config.TRACES_EXTENSION}'):
            trace_filepath_by_serial[serial_dir].append(
                os.path.join(serial_dir_path, entry)
            )
  ...
  def run_analysis(self, filter_by_serials: list[str] = []):
    ...
    Args:
        filter_by_serials (list[str], optional): List of serial numbers to
          filter the traces. Defaults to [].
    """
    trace_filepath_by_serial = self._local_trace_filepath()
    if filter_by_serials:
      trace_filepath_by_serial = {
          serial: trace_filepath_by_serial[serial]
          for serial in filter_by_serials
          if serial in trace_filepath_by_serial
      }
    for serial in trace_filepath_by_serial:
      ...
      for trace_filepath in trace_filepath_by_serial[serial]:
        ...
        for analysis_module_class in self.analysis_module_classes:
          ...
          report_filepath = (
              f'{basename_fullpath}-{analysis_module.module_name}-report.json'
          )
          analysis_module.write_json_results(
              report_filepath, analysis_module_result
          )
