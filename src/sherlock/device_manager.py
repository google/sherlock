...
class DeviceManager:
  ...
  def __init__(
    ...
    self.handled_sherlock_devices: dict[
        str, sherlock_device.ConnectedDevice
    ] = {}
    ...
    while not self.stop_event.is_set():
      ...
      if adb_devices:
        logging.debug(
            ...
            ', '.join([d.serial for d in adb_devices]),
        )
        ...
        for handled_serial_device in list(self.handled_sherlock_devices.keys()):
          if handled_serial_device not in [d.serial for d in adb_devices]:
            logging.debug(
                '%s is not connected, removing from "handled" list',
                handled_serial_device,
            )
        ...
        for device_serial in [d.serial for d in adb_devices]:
          if device_serial not in self.handled_sherlock_devices:
            ...
            connected_device = sherlock_device.ConnectedDevice(
                adb.device(device_serial), self.sherlock_config
            )
            match self.mode:
              case DeviceManagerMode.TERMINATE_COLLECT:
                logging.info(
                    '[%s]: start %s mode', device_serial, self.mode.name
                )
